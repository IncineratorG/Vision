...
public class Test_TestCameraMotionDetectionHandler_V2 implements JSActionHandler {
    ...
    public void handle(ReactApplicationContext context, ReadableMap action, Promise result) {
        ...
        cameraMotionDetectionService.requestBackCameraImage((imageBytes, width, height, previewFormat) -> {
            ...
            Mat rgbaMat = cameraPreviewImageToRgbaMat(imageBytes, width, height, previewFormat);
            if (rgbaMat == null) {
                Log.d("tag", "Test_TestCameraMotionDetectionHandler_V2->handle(): RGBA_MAT_IS_NULL");
                return;
            }
            Mat rotatedRgbaMat = rotateMat(rgbaMat, 270);
//            Mat rotatedRgbaMat = loadTestImageMat(context);
            byte[] processedImageBytes = processImage(context, rotatedRgbaMat);
...
//            byte[] jpegBytes = cameraPreviewImageToJpgBytesArray(imageBytes, width, height, previewFormat);
//            if (jpegBytes == null) {
//                Log.d("tag", "Test_TestCameraMotionDetectionHandler_V2->handle(): JPEG_BYTES_IS_NULL");
//                return;
//            }

//            String base64 = Base64.encodeToString(jpegBytes, Base64.DEFAULT);
//            context
//                    .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
//                    .emit(
//                            "IMAGE_TAKEN_EVENT",
...
//        return buffer.toArray();
    ...
    private byte[] processImage(Context context, Mat jpegMat) {
        ...
        Imgproc.cvtColor(jpegMat, inputFrame, Imgproc.COLOR_RGBA2RGB);
