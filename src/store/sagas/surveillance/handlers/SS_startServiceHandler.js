...
const SS_startServiceHandler = ({channel}) => {
  const handler = function* (action) {
    yield put(AppActions.surveillance.actions.startServiceBegin());
    ...
    try {
      ...
      yield put(AppActions.surveillance.actions.startServiceFinished());
    } catch (e) {
      ...
      yield put(
        AppActions.surveillance.actions.startServiceError({code, message}),
      );
    }
  };
